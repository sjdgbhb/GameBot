...
class SwiftBeastTask(AtomicTaskBase):
    """迅猛野兽原子任务。"""

    _task_label = "迅猛野兽"
    _walk_offset = [0, 100]
    _task_grid_key = "beast_grid"
...
def main():
    ...
    setup_log_file("迅猛野兽任务")
    logger.info("############################# 迅猛野兽任务 #############################")

    cfg = config.load_task("war3.jiubing2.tasks.atomic.swift_beast")
    task_cfg = cfg["war3"]["jiubing2"]["tasks"]["atomic"]["swift_beast"]
    ...
    run_with_float_window("迅猛野兽任务", task_wrapper, countdown_seconds=5, float_cfg=cfg.get("base", {}).get("float_window", {}))
