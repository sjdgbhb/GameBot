...
class InputControllerMixin:
    ...
    def send_msg(self, msg: str, is_english: bool = True, is_all: bool=False,
    ...
    def _click_target(self, target_coords: list, mode: int = 0, wait_time: float = None, stop_event: Optional[threading.Event] = None):
    ...
    def move_to_point(self, target_coords: list, mode: int = 0, wait_time: float = None, stop_event: Optional[threading.Event] = None):
        """
        移动到目标点（当前屏幕可见范围内）
    ...
    def move_to_minimap_point(self, mini_coords, target_coords, mode: int = 0, wait_time: float = None, stop_event: Optional[threading.Event] = None):
        """
    ...
    def go_through_teleport(self, tp: dict, stop_event: Optional[threading.Event] = None):
        ...
        self.move_to_minimap_point(
            tp.get('mini_coords'), tp.get('coords'),
            tp.get('walk_mode', 1), tp.get('time', 3), stop_event,
        )
    ...
    def use_inventory_item(self, hotkey: str,
                            stop_event: Optional[threading.Event] = None):
        """使用背包物品（按快捷键）
