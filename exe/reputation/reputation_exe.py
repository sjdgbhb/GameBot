...
def _load_user_config() -> dict:
    ...
    user_cfg_path = EXE_DIR / "每日声望_config.toml"
...
def main():
    ...
    if user_cfg:
        logger.info(f"已加载用户配置: {EXE_DIR / '每日声望_config.toml'}")

    # 预加载任务配置以获取 float_window 设置
    # 优先级：user_cfg > 任务配置文件 > 默认配置
    pre_cfg = config.load_task("war3.jiubing2.tasks.reputation.daily_reputation")
