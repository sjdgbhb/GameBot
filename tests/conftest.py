...
_MARKERS = {'unit': '单元测试', 'integration': '集成测试（真实文件 I/O，多模块协作）', 'manual': '需人工介入的手动测试', 'dm': '依赖大漠插件 COM（经 dm_bridge 子进程调用）', 'slow': '耗时较长的测试', 'inference': '依赖 ONNX / AI 模型的推理测试', 'web': 'Web API / FastAPI 测试', 'config': '配置系统测试'}
...
def _populate_standard_config(config_dir: Path) -> Path:
    ...
    _write_toml_helper(config_dir, 'war3/jiubing2/jiubing2.toml', '\nname = "war3.jiubing2.jiubing2"\nextends = ["war3"]\n\n[game]\nload_war3_time = 33\n\n[command]\nclear_nearby = "-delh"\n\n[hero]\ninventory = ["A", "B", "C"]\n')
