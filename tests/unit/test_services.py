...
class TestLoadTasks:
    ...
    def test_load_tasks_scans_directory(self, services_config_dir):
        ...
        _write_task(services_config_dir, 'others/fishing.toml', 'name = "war3.jiubing2.tasks.others.fishing"\nextends = ["war3.jiubing2"]\n\n[this]\nname = "钓鱼"\n')
        _write_task(services_config_dir, 'others/patrol_loot.toml', 'name = "war3.jiubing2.tasks.others.patrol_loot"\nextends = ["war3.jiubing2"]\n\n[this]\nname = "巡逻拾取"\n')
    ...
    def test_load_tasks_skips_invalid_toml(self, services_config_dir):
        """损坏的 TOML 文件应被跳过。"""
        _write_task(services_config_dir, 'bad.toml', 'name = "war3.jiubing2.tasks.bad"\nextends = ["war3.jiubing2"]\nnot valid toml [[[')
        _write_task(services_config_dir, 'good.toml', 'name = "war3.jiubing2.tasks.good"\nextends = ["war3.jiubing2"]\n[this]\nname = "好任务"\n')
        result = services.load_tasks()
        ids = [t['id'] for t in result]
        assert 'good' in ids
...
class TestLoadHeroes:
    ...
    def test_load_heroes_extracts_fields(self, services_config_dir):
        ...
        _write_hero(services_config_dir, 'mk.toml', 'name = "war3.jiubing2.heroes.mk"\nextends = ["war3.jiubing2"]\n[hero]\nname = "山丘之王"\nfloor_key = "P"\ninventory = ["A", "B"]\n[[hero.skills]]\nname = "风暴之锤"\n')
        _write_hero(services_config_dir, 'lancer.toml', 'name = "war3.jiubing2.heroes.lancer"\nextends = ["war3.jiubing2"]\n[hero]\nname = "长枪兵"\nfloor_key = "O"\ninventory = ["C"]\n')
    ...
    def test_load_heroes_sorted_by_floor(self, services_config_dir):
        """英雄应按楼层排序（P 在 O 之前）。"""
        _write_hero(services_config_dir, 'z_hero.toml', 'name = "war3.jiubing2.heroes.z_hero"\nextends = ["war3.jiubing2"]\n[hero]\nname = "Z英雄"\nfloor_key = "O"\n')
        _write_hero(services_config_dir, 'a_hero.toml', 'name = "war3.jiubing2.heroes.a_hero"\nextends = ["war3.jiubing2"]\n[hero]\nname = "A英雄"\nfloor_key = "P"\n')
        result = services.load_heroes()
    ...
    def test_load_heroes_name_from_comment(self, services_config_dir):
        ...
        _write_hero(services_config_dir, 'test_hero.toml', '### 山丘之王 ###\nname = "war3.jiubing2.heroes.test_hero"\nextends = ["war3.jiubing2"]\n[hero]\nfloor_key = "P"\ninventory = []\n')
        result = services.load_heroes()
...
class TestUserConfigs:
    ...
    def test_load_user_configs_old_multi_format(self, services_user_config_path):
        ...
        services_user_config_path.write_text('{"active": "profile1", "configs": {"profile1": {"hero": "lancer"}, "profile2": {"hero": "mk"}}}', encoding='utf-8')
        result = services.load_user_configs()
...
class TestStartTask:
    ...
    def test_start_task_success(self, services_config_dir, services_project_root, services_web_config, monkeypatch):
    ...
    def test_start_task_already_running(self, services_config_dir, services_project_root, services_web_config, monkeypatch):
        """已在运行的任务应返回 ok=False。"""
...
class TestLoadTaskDefaults:
    ...
    def test_load_defaults_hero_from_dependencies(self, services_config_dir):
        """应从 extends 中提取英雄 ID。"""
        _write_task(services_config_dir, 'others/test_task.toml', 'name = "war3.jiubing2.tasks.others.test_task"\nextends = ["war3.jiubing2", "war3.jiubing2.heroes.lancer"]\n\n[this]\nname = "测试"\n')
        _write_hero(services_config_dir, 'lancer.toml', 'name = "war3.jiubing2.heroes.lancer"\nextends = ["war3.jiubing2"]\n[hero]\nname = "长枪兵"\nfloor_key = "P"\ninventory = []\n[[hero.skills]]\nname = "穿刺"\n')
        _write_hero(services_config_dir, 'mk.toml', 'name = "war3.jiubing2.heroes.mk"\nextends = ["war3.jiubing2"]\n[hero]\nname = "山丘"\nfloor_key = "P"\ninventory = []\n[[hero.skills]]\nname = "风暴之锤"\n')
...
class TestImportHeroBatch:
    ...
    def test_import_multiple_heroes(self, services_config_dir):
        ...
        heroes_dir = services_config_dir / 'war3' / 'jiubing2' / 'heroes'
        heroes_dir.mkdir(parents=True)
        result = services.import_hero_batch([{'hero_id': 'hero_a', 'content': '[hero]\nname = "A"\n'}, {'hero_id': 'hero_b', 'content': '[hero]\nname = "B"\n'}])
    ...
    def test_import_mixed_success_and_error(self, services_config_dir):
        ...
        heroes_dir = services_config_dir / 'war3' / 'jiubing2' / 'heroes'
        heroes_dir.mkdir(parents=True)
        result = services.import_hero_batch([{'hero_id': 'good_hero', 'content': '[hero]\nname = "好"\n'}, {'hero_id': 'bad_hero', 'content': 'invalid [[['}])
...
class TestSaveHeroInventory:
    ...
    def test_save_inventory_filters_empty_slots(self, services_config_dir):
        ...
        services.save_hero_inventory('test', [{'id': 1, 'hotkey': '1'}, {'id': -1, 'hotkey': '2'}, {'id': None, 'hotkey': '3'}])
