...
class _UserEdgeConfigTestBase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        _write_toml(self.config_dir, 'base.toml', '\nname = "base"\nextends = []\n[paths]\nresources_path = "resources"\n')
        _write_toml(self.config_dir, 'war3/war3.toml', '\nname = "war3.war3"\nextends = ["base"]\n[war3]\nwindow_title = "Warcraft III"\n')
        _write_toml(self.config_dir, 'war3/jiubing2/jiubing2.toml', '\nname = "war3.jiubing2.jiubing2"\nextends = ["war3"]\n[hero]\ninventory = ["A", "B", "C"]\n')
        _write_toml(self.config_dir, 'war3/jiubing2/heroes/mk.toml', '\nname = "war3.jiubing2.heroes.mk"\nextends = ["war3.jiubing2"]\n[hero]\ninventory = ["D", "E", "F"]\nattack = 100\n')
        _write_toml(self.config_dir, 'war3/jiubing2/tasks/reputation/daily_reputation.toml', '\nname = "war3.jiubing2.tasks.reputation.daily_reputation"\nextends = ["war3.jiubing2"]\n\n[this.blackstone]\nname = "黑石声望"\n\n[this.forest]\nname = "森林声望"\n')
...
class TestLoadUserConfigEdge(_UserEdgeConfigTestBase):
    ...
    def test_old_multi_config_active_format(self):
        ...
        (self.cfg.project_root / 'user_configs.json').write_text(json.dumps({'active': 'custom', 'configs': {'default': {'inventory': ['A']}, 'custom': {'inventory': ['B', 'C']}}}), encoding='utf-8')
...
class TestApplyUserOverridesEdge(_UserEdgeConfigTestBase):
    ...
    def test_hero_configs_points_dict_with_blackstone_and_forest(self):
        ...
        user_cfg = {'combat_mode': 'cast_skills', 'hero': 'lancer', 'hero_configs': {'lancer': {'points': {'blackstone_points': [[10, 20], [30, 40]], 'forest_points': [[50, 60], [70, 80]]}, 'inventory': ['G', 'H', 'I']}}}
    ...
    def test_combat_mode_sync_to_daily_reputation_subtasks(self):
        ...
        config = {'war3': {'jiubing2': {'tasks': {'reputation': {'daily_reputation': {'blackstone': {'name': '黑石'}, 'forest': {'name': '森林'}}}}}}}
