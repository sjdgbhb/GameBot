...
| 模型 | 类型 | 用途 | 输入 | 输出 |
|------|------|------|------|------|
| 宝箱检测 | YOLOv8 目标检测 | 检测地面宝箱位置 | 全屏截图 | 宝箱位置框 |
| 战斗状态 | CNN 二分类 | 判断英雄是否在战斗中 | 英雄头像区域连续截帧 | 战斗概率（0~1） |

模型文件位于 `src/GameBot/resources/models/`，纳入版本控制。
...
- [推理模块](../modules/inference.md) —— 模型如何被加载和使用
- [配置系统](../modules/config.md) —— 模型参数配置（置信度/阈值/输入尺寸）
